...
import { JR_FUNC, PREFIX, ICON, INCLUDE_TYPE, OUTPUT_JSON_INDENT, OUTPUT_FILEPATH_MIN_CHARS } from './constants.js';
...
if (process.argv.length !== Object.keys(ARGS).length) {
  const errorStr = [
    colors.brightRed(`${ICON.ERROR} generate.js was invoked incorrectly`),
    colors.red('  Expected:') + "\tnode {appPath}/generate.js {inputJsonFile/Directory} {outputFile/Directory}",
  ...
  includePaths = [].concat(inputFilePath, process.env.jIncludeDirs).filter(includePath => !!includePath);
...
function processInclude(json, prop, propData) {
   ...
   * VALUE based include
   */
  if (typeof propData === 'string' && propData.substring(0, PREFIX.include.length) === PREFIX.include) {
    const relPath = propData.substring(PREFIX.include.length);
   ...
   * KEY based include
   */
  else if (prop.substring(0, PREFIX.include.length) === PREFIX.include) {
    const relPath = prop.substring(PREFIX.include.length).trim();
    const fn = getIncludedFilePath(relPath);
    ...
    if (fn.length) {
      ...
      const inclJsonMaps = Object.keys(inclJson).filter(iProp => iProp.substring(0, PREFIX.map.length) === PREFIX.map);
      ...
      switch (propData) {
        ...
        case INCLUDE_TYPE.PERMUTE: {
          ...
          for (const iProp of Object.keys(inclJson)) {
            ...
            if (json.hasOwnProperty(iProp)) {
              ...
              if (iProp.substring(0, PREFIX.map.length) === PREFIX.map) {
        ...
        default: { // "DEFAULTS" should be the default case
          ...
          // (non-map) property in the root JSON file
          for (const iMapProp of inclJsonMaps) {
            const iBaseProp = iMapProp.substring(PREFIX.map.length);
            if (json.hasOwnProperty(iBaseProp)) {
...
function parseIncludes(json) {
  if (json !== null && json !== undefined) {
    for (const [prop, propData] of Object.entries(json)) {
      ...
      if (
          prop.substring(0, PREFIX.include.length) === PREFIX.include ||
          typeof propData === 'string' && propData.substring(0, PREFIX.include.length) === PREFIX.include
      ) {
      ...
      // an Array,
        ...
        if (JSON.stringify(propData).indexOf(`"${PREFIX.include}`) >= 0) {
          for (const [idx, item] of Object.entries(propData)) {
            ...
            // an include, process it
            else if (typeof item === 'string' && item.substring(0, PREFIX.include.length) === PREFIX.include) {
