...
/**
 * Swaps Sentinels in JSON keys
 * @param {Object} json - JSON to parse
 * @param {Object} [opts] - Optional params
 * @param {Boolean} [opts.reverse=false] - Sort output JSON objects/files by key
 * @returns {Object}
 */
function swapSentinelsInKeys(json, { reverse } = { reverse: false }) {
  let stringJson = JSON.stringify(json);
  let stringJsonFmt = JSON.stringify(json, null, 2);
  const matchFunctions = `${PREFIX.map}|${PREFIX.mapZipper}|${PREFIX.mapKey}|${PREFIX.concat}|${PREFIX.zipperMerge}|${PREFIX.comment}`;
  const noMatchFunctions = `${JR_FUNC.mapAllowOnly}|${JR_FUNC.mapExclude}|${JR_FUNC.include}|${JR_FUNC.comment}|${JR_FUNC.filename}`;
  const dotMatchRegex = new RegExp(`^\\s+"(?!${noMatchFunctions})(${matchFunctions})?(.+?)":`, 'gm');
  const dotMatches = stringJsonFmt.match(dotMatchRegex);

  if (dotMatches) {
    for (let dotMatch of dotMatches) {
      dotMatch = dotMatch.trim();
      for (const [char, sentinel] of SENTINEL_REPLACEMENT_MAPPING) {
        if (reverse) {
          stringJson = stringJson.replace(
            new RegExp(`${dotMatch}`, 'g'),
            dotMatch.trim().replace(new RegExp(`${escapeRegExp(sentinel)}`, 'g'), char)
          );
        } else {
          stringJson = stringJson.replace(
            new RegExp(`${dotMatch}`, 'g'),
            dotMatch.trim().replace(new RegExp(`${escapeRegExp(char)}`, 'g'), sentinel)
          );
        }
      }
    }
...
export default function parseLayout(json, { sortKeys } = { sortKeys: true }) {
  ...
  const layoutPermutations = [ cloneDeep(swapSentinelsInKeys(json)) ];
  ...
  // remove comments
  cleanedPermutations = cleanedPermutations.map(p => removeComments(swapSentinelsInKeys(p, { reverse: true })));
