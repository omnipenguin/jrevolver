...
function _parseMaps(json, opts = { path: '' }) {
  ...
  // filter generated permutations using the white/blacklist
  const hasBlacklist = globalBlacklist && globalBlacklist.length;
  const hasWhitelist = globalWhitelist && globalWhitelist.length;

  for (const [p, perm] of Object.entries(expandedPermutations)) {
    const inBlacklist = hasBlacklist && objMatchFromList(perm, globalBlacklist);
    const inWhitelist = hasWhitelist && objMatchFromList(perm, globalWhitelist);

    if (inBlacklist || (hasWhitelist && !inWhitelist)) {
...
export default function parseMaps(json) {
  ...
  // merge and build global white/blacklists
  [JR_FUNC.mapBlacklist, JR_FUNC.mapWhitelist]
      .forEach(mapListName => {
  ...
  globalBlacklist = parseMapList(json[JR_FUNC.mapBlacklist]);
  globalWhitelist = parseMapList(json[JR_FUNC.mapWhitelist]);
  delete json[JR_FUNC.mapBlacklist];
  delete json[JR_FUNC.mapWhitelist];
