...
import { ICON, JR_FUNC, PREFIX, MAP_CURRENT_CONTEXT_SENTINEL } from './constants.js';
import {
  uniqArray,
...
function keyIsMap(key) {
  return (typeof key === 'string') && key.substring(0, PREFIX.map.length) === PREFIX.map;
}
...
function keyIsMapKey(key) {
  return (typeof key === 'string') && key.substring(0, PREFIX.mapKey.length) === PREFIX.mapKey;
}
...
function _parseMaps(json, opts = { path: '' }) {
  ...
  for (const [key, item] of Object.entries(json)) {
    ...
    const maybeMapName = keyIsMap(key) && key.substring(PREFIX.map.length);
    /**
...
function validateMapList(json, mapListName) {
  ...
  if (isArray(mapList)) {
    for (const [, item] of Object.entries(mapList)) {
      if (!isObject(item)) {
        throw new Error(
          colors.brightRed(`${ICON.ERROR} Invalid map ${mapListName} entry:`) + `${JSON.stringify(item)}\n` +
    ...
    throw new Error(
      colors.brightRed(`${ICON.ERROR} Invalid map ${mapListName}:`) + `${JSON.stringify(mapList)}\n` +
...
function replaceMapKeys(json) {
  if (isObject(json)) {
    for (const [key, item] of Object.entries(json)) {
      if (keyIsMapKey(item)) {
        const mapKeyName = item.substring(PREFIX.mapKey.length);
