...
export function jrFuncBaseProp(key) {
  const funcName = (key.substring(0, 2) === JR_FUNC_PREFIX && key.substring(0, key.indexOf(' '))) || null;
  const baseProp = funcName ? key.substring(key.indexOf(' ') + 1) : key;

  if (funcName && !Object.values(JR_FUNC).includes(funcName)) {
    throw new Error(`${colors.brightYellow(funcName)} ${colors.yellow('is not a valid function')}`);
  }
