...
export function mergeDeep(target, ...sources) {
  ...
  for (const [key,] of Object.entries(source)) {
    if (isObjectNotArray(source[key])) {
      ...
      _.assignInWith(target, { [key]: source[key] }, function(objValue, srcValue, key, object, source) { // eslint-disable-line no-unused-vars
...
export function setPropByPath(obj, path, value) {
  ...
  for (let p = 0; p < pathList.length; p++) {
    ...
    if (pointer[curProp] === undefined) {
      ...
      if (isNumber(nextProp)) pointer[curProp] = []; // Array
      else pointer[curProp] = {}; // Object
    }
  ...
  if (JSON.stringify(json).includes(JR_FUNC.comment)) {
    if (isArray(json)) {
      for (const [i, v] of Object.entries(json)) {
        if (typeof v === 'string' && v.substr(0, JR_FUNC.comment.length) === JR_FUNC.comment) {
          json.splice(i, 1);
        }
      }
    }
    ...
    for (const [k, v] of Object.entries(json)) {
      if (k.substr(0, JR_FUNC.comment.length) === JR_FUNC.comment) {
        delete json[k];
      } else if (isObject(v)) {
...
export function getSelfCmd() {
  ...
  return cmd.substr(cmd.indexOf(' ')-4);
}
