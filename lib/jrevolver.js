...
import { MOCK_FILE_EXTENSION, EXCLUDE_DIRECTORY_FILE } from './constants.js';
...
mockList.forEach(function(filePath) {
  ...
  shell.exec(`node --experimental-modules --no-warnings ${modulePath}/generate.js ${mockPath} ${outputPath}`, { env: process.env });
});
console.log();
...
function buildMockList(targetDir) {
  ...
  files.forEach(function(file) {
    ...
    if (fs.statSync(filePath).isDirectory()) {
      // ignore directory if it contains a ".exclude" file
      if (!fs.existsSync(`${filePath}/${EXCLUDE_DIRECTORY_FILE}`)) {
