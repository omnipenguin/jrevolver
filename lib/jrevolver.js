...
if (process.argv.length < Object.keys(ARGS).length-1) {
  const errorStr = ['',
    colors.brightRed(`${ICON.ERROR} jRevolver.js was invoked incorrectly`),
    colors.red('  Expected:') + "\tnode {appPath}/jrevolver.js {rootMockDirectory} {...additionalIncludeDirectories*}",
  ...
  scriptFile  = process.argv[ARGS.script]; // eslint-disable-line no-unused-vars
